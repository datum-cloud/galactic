...
| `GALACTIC_CNI_SRV6_FABRIC_NEXTHOPS`    | comma-separated IPv6 prefixes | _(none)_ | Addresses of the fabric's BGP peers as route gateways: link and transit addresses, `fe80::/10` for unnumbered sessions. When set, a route counts only through a hop to one of them on an uplink. Unset skips the check. |
...
- `galactic-router` tags its routes with protocol 214 from this release on, and re-tags routes an older version installed when it restarts and replays its RIB. Upgrade it before enabling `audit`, or its gateway and egress shard routes will not be allowed yet.
