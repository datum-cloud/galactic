...
- Argument-based uSID reservation: Argument 0 is already reserved as "this uSID means the gateway's own self-address" (`NetworkGatewayStatus.SRv6Address`, uformat's `ArgumentMin`/`ArgumentMax`). This plan reserves a second Argument *range* on a shared `egress_sid` locator — each value in the range identifies a distinct tenant/VRF, mirroring how #791 (NAT64) isolates tenants on its own shared locator — rather than a single additional reserved constant. See §3.1/§3.2 for why a single shared constant is unsafe here.
...
Three distinct address *families* are needed per gateway node, not two:

| Address      | Kind                                                                  | Meaning                                                                                                                                                                                |
|--------------|------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `gw_addr`    | SRv6 uSID, Argument 0 (existing)                                        | Ingress Full-NAT SNAT source / return-branch match. Unchanged.                                                                                                                          |
| `egress_sid` | SRv6 uSID **locator**, tenant/VRF identity carried in the Argument bits | Where a tenant VRF's default route points. Decap match for a *fresh* outbound flow. Dispatch matches on the locator only (mask off the Argument bits); the unmasked Argument value is the flow's tenant/VRF identifier, extracted per-packet with no new map lookup. |
...
`egress_sid` **cannot be a single reserved constant Argument value** the way `gw_addr`'s Argument 0 is. #865's own motivation is that tenant ULA space is not guaranteed unique (independent orgs' RFC 4193 self-generated prefixes can collide) — so an egress conn-table keyed only on `(proto, backend_addr:port, dest_addr:port)` would let two different tenants with the same (colliding) internal ULA source address and the same destination collide in the same table row. Treating `egress_sid`'s Argument bits as a tenant/VRF discriminator — exactly the mechanism #791 (NAT64) already uses for its own shared locator — closes that hole with a masked-address dispatch match instead of a per-packet map lookup, so there's no new cost on the fast path.
...
edge_nat(ctx):
  ...
  if (daddr & egress_locator_mask) == egress_sid:
                                           handle_egress_forward(...)  # NEW — tenant_arg = daddr's Argument bits
...
Four-way branch on three configured address/mask pairs, read once per packet from two maps — same O(1) shape as today's two-way branch; extracting `tenant_arg` from `daddr` is a mask-and-shift already available via the existing uSID Locator/Argument split helpers (`internal/plumbing/intf`), not an added lookup — no regression to existing ingress performance.
...
- `egress_conn_table` (`BPF_MAP_TYPE_LRU_HASH`, self-evicting): a **new** struct, not a repurposed `conn_table`/`conn_value`. `conn_value`'s fields (`client_addr`, `vip_addr`, `backend_addr`, `gw_addr`) are named for the ingress direction and don't map cleanly onto an egress flow's shape (there is no "client" or "VIP" in an egress flow — there's a backend's own address, an arbitrary internet destination, and the masquerade address). A new `egress_conn_value{ tenant_arg, backend_addr, backend_port, backend_usid, dest_addr, dest_port, masq_addr, masq_port, proto }` keeps every map's field names honest instead of overloading a shared struct with two meanings — the same reasoning `edgeprog`'s own `doc.go` gives for being a separate package from `internal/plumbing/ebpf/prog` rather than a second program folded into it. `tenant_arg` (the Argument bits extracted from the packet's `egress_sid` destination address, §3.1) is part of the **forward-direction key** — `(proto, tenant_arg, backend_addr:backend_port → dest_addr:dest_port)` — because `backend_addr` alone is not guaranteed globally unique; the reverse-direction key, `(proto, dest_addr:dest_port → masq_addr:masq_port)`, does not need it, since `masq_port` is already allocated uniquely per `masq_addr` by the SNAT-port claim itself. `tenant_arg` is still carried in `egress_conn_value` alongside the reverse row so the two directions share one struct shape.
- **No new per-tenant *policy* table is proposed for the datapath** — see the decision in §7.2. *Enablement* (should this tenant reach `egress_sid` at all) is still resolved entirely by routing: whether a tenant VRF's default route toward `egress_sid` exists (a control-plane decision, made once per tenant), not by a per-packet map lookup. This still generalizes decision #1 of the existing ingress design ("a VIP is globally unique by construction, no tenant dimension needed in the datapath") for the enablement question. *Isolation* (don't let two tenants' sessions collide once both are enabled) is a separate question the ingress precedent doesn't have to answer — a VIP is centrally allocated and so is globally unique by construction, but a tenant's ULA source address is not. That's what `tenant_arg` in the conn-table key (above) resolves, and it costs no new map: the Argument bits are already present on every packet that reaches `handle_egress_forward` at all.

### 3.3 New packet paths

**`handle_egress_forward`** — triggered when the outer destination's locator matches `egress_sid` and outer `nexthdr == 41` (the same plain IPv6-in-IPv6 wire format every other cross-node SRv6 packet in this codebase uses, so a tenant VRF's default route needs zero new encap format). Extract `tenant_arg` from the outer destination address's Argument bits *before* discarding it (reuse the existing uSID Locator/Argument split helpers — same convention `gw_addr`'s Argument 0 already uses). Strip the outer header (reuse `strip_outer_header` verbatim), parse the inner IPv6 + L4 header, look up `egress_conn_table` by the forward key `(proto, tenant_arg, backend_addr:backend_port → dest_addr:dest_port)`. A miss on a SYN or UDP packet allocates: pick a `masq_port` via the existing linear-probe technique, claim the reverse row `(proto, dest_addr:dest_port → masq_addr:masq_port)` with `BPF_NOEXIST`, remember `tenant_arg`/`backend_addr`/`backend_usid` in the `conn_value` (needed to route the eventual reply back to the originating worker node). SNAT `saddr` to `masq_addr:masq_port`, fix the L4 checksum, `XDP_TX` back out `ctx->ingress_ifindex` as a **plain IPv6 packet — no outer header pushed**. This is the one genuinely new tail shape in the file: `handle_forward` ends by pushing an outer header, `handle_return` ends by having already stripped one; this path ends by stripping one and sending the *inner* packet on, unwrapped, toward the real internet.

**`handle_egress_return`** — triggered when the destination matches `masq_addr` (a plain address match, no `nexthdr == 41` requirement — this arrives as an ordinary internet-originated IPv6 packet, not an SRv6-encapsulated one, mirroring `handle_forward`'s "match a plain destination address" shape rather than `handle_return`'s "must be an SRv6-encapsulated reply" shape). Parse L4, look up `egress_conn_table` by the reverse key `(proto, dest_addr:dest_port → masq_addr:masq_port)` — no `tenant_arg` needed in this direction's key, since `masq_addr:masq_port` is already unique per flow by construction (§3.2). A miss drops (claimed address, no pass-through, same fail-closed convention every other claimed-address branch in this file already uses). A hit DNATs `daddr` to `backend_addr:backend_port`, fixes the checksum, and pushes a fresh 40-byte outer SRv6 header addressed to `backend_usid` (reuse `push_outer_header` verbatim — this is exactly the return-trip mirror of `handle_egress_forward`), then `XDP_TX`. `backend_usid` (not `backend_addr`) is what makes this direction safe despite two tenants potentially sharing the same `backend_addr`: it's the pod's own fabric-wide SRv6 address, already tenant/VRF-scoped by construction, so it was never ambiguous — the collision risk this plan closes is specifically the *forward*-direction lookup, which is the one place a raw, possibly-colliding ULA address was used as a key on its own.
...
2. **Per-tenant *enablement* enforcement in the datapath vs. enforcement-by-routing-existence-only (§3.2) — resolved toward routing-only.** Whether a tenant can reach `egress_sid` at all stays a routing-layer decision (does their VRF have the default route), not a per-packet lookup; revisit only if a real abuse/quota need surfaces later. This is a narrower question than it looks — it does *not* mean "no tenant dimension in the datapath" for egress: the conn-table key still carries `tenant_arg` for *isolation* (§3.1/§3.2), which is a correctness requirement (ULA collision), not a policy-enforcement one, and was folded into this plan after #865 was rewritten to call out the collision risk explicitly.
