...
// a per-network egress instruction: a network that stops naming a shard must
...
func TestEgressDefaultRouteWithdraw_RemovesBothPrefixes(t *testing.T) {
	...
	// Twice, to pin that a second withdrawal is not an error: every
	// attachment ADD for a network with no egress runs this path.
	for range 2 {
