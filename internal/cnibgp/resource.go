...
type resourceTracker struct {
	...
	// vrfInstanceCreated is only ever true when this ADD's own CreateOrUpdate
	// call reported OperationResultCreated for the (VPC/node-shared)
	// BGPVRFInstance — see publishResult's doc comment. That is the one case
	// where deleting it on rollback is safe: no sibling attachment could
	// have started depending on a CRD this exact ADD invocation just
	// created moments ago, and leaving it behind after this same ADD's own
	// checkArgumentCollision rejected it would permanently wedge every
	// retry on the identical collision (nothing else ever clears it).
