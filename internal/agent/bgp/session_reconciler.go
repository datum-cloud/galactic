...
const (
	...
	SessionFinalizer = "bgp.galactic.datumapis.com/session-cleanup"
)
...
func (r *SessionReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	thisNodeEndpoint := "node-" + r.NodeName
	...
	lastAddrAnnotation := "bgp.galactic.datumapis.com/last-gobgp-address-" + r.NodeName
...
func (r *SessionReconciler) handleDelete(ctx context.Context, c gobgpapi.GobgpApiClient, session *bgpv1alpha1.BGPSession) error {
	...
	addr := ""
	nodeAnnotationKey := "bgp.galactic.datumapis.com/last-gobgp-address-" + r.NodeName
