...
//
// srv6Net is this node's own /48 prefix (from SRV6_NET env var); routes matching
// it are skipped so the node does not install a route to itself.
//
// This function blocks until ctx is cancelled.
func RunRouteWatcher(ctx context.Context, gobgp *GoBGPClient, srv6Net string) {
	...
	for {
		...
		for _, path := range table.Paths {
			...
			if path.IsWithdraw {
				...
				if err := underlay.DelRoute(prefix); err != nil {
				...
				if err := underlay.AddRoute(prefix, nextHop); err != nil {
